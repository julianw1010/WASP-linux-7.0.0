...
void pgtable_repl_set_pte(pte_t *ptep, pte_t pteval)
{
	...
	while (cur_page && cur_page != start_page) {
		...
		replica_entry = (pte_t *)(page_address(cur_page) + offset);
		WRITE_ONCE(*replica_entry, pte_mkold(pteval));
		cur_page = READ_ONCE(cur_page->pt_replica);
	}

	/* Ensure all replica writes are visible */
	smp_wmb();
	return;
...
void pgtable_repl_set_pmd(pmd_t *pmdp, pmd_t pmdval)
{
	...
	if (has_child) {
		...
		if (child_phys && pfn_valid(child_phys >> PAGE_SHIFT)) {
			...
			if (child_page &&
			    virt_addr_valid(page_address(child_page))) {
				child_base_page = child_page;
				child_has_replicas =
					READ_ONCE(child_base_page->pt_replica)
					!= NULL;
			}
		}
	}
	...
	if (pmd_present(old_primary) &&
	    ...
	    (pmd_trans_huge(pmdval) || pmd_leaf(pmdval) ||
	     !pmd_present(pmdval))) {
		unsigned long old_child_phys;

		old_child_phys = pmd_val(old_primary) & pfn_mask;
		if (old_child_phys &&
		    pfn_valid(old_child_phys >> PAGE_SHIFT)) {
			struct page *primary_pte;
			struct page *repl_cur;
			struct page *repl_start;
			struct mm_struct *owner;

			primary_pte = pfn_to_page(
				old_child_phys >> PAGE_SHIFT);
			repl_cur = xchg(&primary_pte->pt_replica, NULL);
			repl_start = primary_pte;

			while (repl_cur && repl_cur != repl_start) {
				struct page *repl_next;

				repl_next = READ_ONCE(repl_cur->pt_replica);
	...
	do {
		...
		replica_entry = (pmd_t *)(page_address(cur_page) + offset);

		if (has_child && child_has_replicas && child_base_page) {
			struct page *local;

			local = get_replica_for_node(child_base_page, node);
			if (local) {
				unsigned long phys;

				phys = __pa(page_address(local));
				node_val = phys | (entry_val & ~pfn_mask);
			} else {
				node_val = entry_val;
			}
		} else {
...
void pgtable_repl_set_pud(pud_t *pudp, pud_t pudval)
{
	...
	has_child = pud_present(pudval) && !pud_trans_huge(pudval) &&
		    entry_val != 0;
	...
	if (has_child) {
		...
		child_has_replicas =
			READ_ONCE(child_base_page->pt_replica) != NULL;
	}
	...
	do {
		...
		replica_entry = (pud_t *)(page_address(cur_page) + offset);
...
void pgtable_repl_set_p4d(p4d_t *p4dp, p4d_t p4dval)
{
	...
	if (!READ_ONCE(parent_page->pt_replica)) {
		...
		if (!pgtable_l5_enabled() && mitosis_pti_active()) {
			user_entry = mitosis_get_user_pgd_entry(
				(pgd_t *)p4dp);
			if (user_entry)
				WRITE_ONCE(*user_entry,
					   __pgd(p4d_val(p4dval)));
		}
		return;
	}
	...
	do {
		...
		replica_entry = (p4d_t *)(page_address(cur_page) + offset);
...
void pgtable_repl_set_pgd(pgd_t *pgdp, pgd_t pgdval)
{
	...
	if (!READ_ONCE(parent_page->pt_replica)) {
		...
		if (mitosis_pti_active()) {
			user_entry = mitosis_get_user_pgd_entry(pgdp);
			if (user_entry)
				WRITE_ONCE(*user_entry,
					   __pgd(pgd_val(pgdval)));
		}
		return;
	}
	...
	do {
		...
		replica_entry = (pgd_t *)(page_address(cur_page) + offset);
...
pte_t pgtable_repl_get_pte(pte_t *ptep)
{
	...
	while (cur_page && cur_page != start_page) {
		...
		replica_pte = (pte_t *)(page_address(cur_page) + offset);
...
pte_t pgtable_repl_ptep_get_and_clear(struct mm_struct *mm, pte_t *ptep)
{
	...
	do {
		...
		replica_entry = (pte_t *)(page_address(cur_page) + offset);
		old_entry = native_ptep_get_and_clear(replica_entry);
		val |= pte_val(old_entry);
		cur_page = READ_ONCE(cur_page->pt_replica);
	} while (cur_page && cur_page != start_page);

	/* Ensure all clears are visible */
	smp_wmb();
	return __pte(val);
}
				     ...
				     unsigned long addr, pte_t *ptep)
{
	...
	if (!READ_ONCE(pte_page->pt_replica)) {
		...
		} while (!try_cmpxchg((long *)&ptep->pte,
				      (long *)&old_pte,
				      *(long *)&new_pte));
		return;
	}
	...
	do {
		...
		do {
			new_pte = pte_wrprotect(old_pte);
		} while (!try_cmpxchg((long *)&replica_entry->pte,
				      (long *)&old_pte,
				      *(long *)&new_pte));
		cur_page = READ_ONCE(cur_page->pt_replica);
	} while (cur_page && cur_page != start_page);

	/* Ensure all wrprotect updates are visible */
	smp_wmb();
	return;
					   ...
					   pte_t *ptep)
{
	...
	/* Ensure all accessed bit clears are visible */
	smp_wmb();
	return young;
					    ...
					    pmd_t *pmdp)
{
	...
	if (pmd_present(__pmd(val)) &&
		...
		old_child_phys = val & PTE_PFN_MASK;
		if (old_child_phys &&
		    pfn_valid(old_child_phys >> PAGE_SHIFT)) {
			struct page *primary_pte;
			struct page *repl_cur;
	...
	while (cur_page && cur_page != start_page) {
		...
		replica_entry = (pmd_t *)(page_address(cur_page) + offset);
		old_entry = native_pmdp_get_and_clear(replica_entry);
				     ...
				     unsigned long addr, pmd_t *pmdp)
{
	...
	while (cur_page && cur_page != start_page) {
		...
		if (pmd_present(*replica_entry)) {
			...
			do {
				new_pmd = pmd_wrprotect(old_pmd);
			} while (!try_cmpxchg((long *)replica_entry,
					      (long *)&old_pmd,
					      *(long *)&new_pmd));
		}
...
void pgtable_repl_free_pte_replicas(struct mm_struct *mm, struct page *page)
{
	...
	/* Ensure all replica pointers are cleared before freeing */
	smp_mb();
	...
	for (i = 0; i < free_count; i++) {
		...
		if (from_cache) {
			...
			if (mitosis_cache_push(pages_to_free[i], nid,
					       MITOSIS_CACHE_PTE))
				continue;
		}
				  ...
				  pmd_t pmd)
{
	...
	while (cur_page && cur_page != start_page) {
		...
		if (IS_ENABLED(CONFIG_SMP)) {
			old_repl = xchg(replica_entry, repl_pmd);
		} else {
	...
	/* Ensure all replica updates are visible */
	smp_wmb();
	return __pmd(val);
